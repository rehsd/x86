...
ide_identify_drive:
	...
	;;mov		ax,		0x0000
	;;mov		ds,		ax				; ds:bx buffer to print
	;mov		cx,		128				; number of words to print
	;mov		bx,		drive_buffer	; temp - change to 0x7c00
...
isr_int_13h:				; disk services ISR
	...
	.ext_read_sectors:					;0x42
		...
		.ok:
			...
			mov		al,						ds:[si+D_LBA+3]		; bits 27..24 of LBA			+1 for little endian
			and		al,						0b00001111			; probably not needed, but just in case upper nibble is not right
			or		al,						0b11100000			; set bit 6 for LBA mode		assuming drive 0x80 for now
			out		IDE_REG_DRIVE_HEAD,		al
			...
			mov		al,						ds:[si+D_LBA+2]		; bits 23..16 of LBA
			out		IDE_REG_CYL_HIGH,		al
			;****** DEBUG ***********************
				call	print_char_hex_spi
				mov		al, ':'
			...
			mov		al,						ds:[si+D_LBA+1]		; bits 15..8 of LBA
			out		IDE_REG_CYL_LOW,		al
			...
			mov		al,						ds:[si+D_LBA+0]		; bits 7..0 of LBA
			out		IDE_REG_SECT_NUMBER,	al
	...
	.ext_read_drive_params:				;0x48
		...
		mov		ax,						0x001e							;size of reult buffer = 0x1e	
		mov		word ds:[si+0],			ax								
