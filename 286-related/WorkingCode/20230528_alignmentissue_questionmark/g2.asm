...
; Assembler: NASM
; Target clock: Tested PCB v0.20 with 2 to 11 MHz CPU clock
;
; *physical memory map*
; -------------------------------
; -       ROM  (256 KB)			-
...
section .text align=GLOBAL_ALIGNMENT
	top:				; physically at 0xC0000
		...
		call	print_last_bss_var_location
...
init_bda:
	...
	mov		ax,					[kbd_buff]
	mov		[kbd_buff_tail],	ax
	mov		di,				0
	.clear_kbd_buffer:
		mov		word [kbd_buff_head+di],		0x0000
		add		di, 2
		cmp		di, 32
	...
	mov		[kbd_buff_start_offset],	word 0x001e			;??? is tis correct?
	mov		[kbd_buff_end_offset],		word 0x003d			;??? is tis correct?
...
section .rodata start=0x30000	align=GLOBAL_ALIGNMENT
%include "romdata.asm"
