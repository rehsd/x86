...
spi_init:
	...
	mov		al,			(SPI_CS1 | SPI_SCK | SPI_MOSI)	; set bits as output -- other bits will be input
	out		VIA1_DDRB,	al
	;call	delay
...
spi_writebyte:
	...
	.sendbit:
		...
		xor		al,				SPI_SCK
		out		VIA1_PORTB,		al			; raise SCK keeping MOSI the same, to send the bit
		;call	delay
...
spi_readbyte:
	...
	.loop:
		...
		out		VIA1_PORTB,		al
		mov		al,				(SPI_MOSI | SPI_SCK)		; toggle the clock high
		out		VIA1_PORTB,		al
...
spi_send_LCDcmd:
	...
	mov		al,				(SPI_MOSI)					; drop SPI_CS1 low
	out		VIA1_PORTB,		al		
	; call	delay
	...
	mov		al,				(SPI_CS1 | SPI_MOSI)		; bring SPI_CS1 high
	out		VIA1_PORTB,		al	
	; call	delay
