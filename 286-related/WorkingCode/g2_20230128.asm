...
; Latest update: VGA 640x480 test screen
; Assembler: NASM
...
vga_draw_rect:
	; params: (subs use vga_param_color), vga_rect_start_x, vga_rect_start_y, vga_rect_end_x, vga_rect_end_y
	; *currently start vals must be lower than end vals
	; TO DO bounds checks
	...
	inc		word [vga_rect_end_x]		; param - number is inclusive of rect - add 1 to support loop code strucure below
	inc		word [vga_rect_end_y]		; param - number is inclusive of rect - add 1 to support loop code strucure below

	mov		bx,					[vga_rect_start_y]
	mov		[vga_param_y],		bx
	mov		ax,					[vga_rect_start_x]
	.row:
		mov		[vga_param_x],		ax
		call	vga_draw_pixel
		inc		ax										; move right a pixel
		cmp		ax,					[vga_rect_end_x]
		jne		.row
	mov		ax,					[vga_rect_start_x]		; start over on x position
	inc		bx											; move down a pixel
	mov		[vga_param_y],		bx
	cmp		bx,					[vga_rect_end_y]
	jne		.row
...
vga_set_segment_from_xy:
	; updates VGA register to set active segment based on vga_param_x & vga_param_y coordinates
	push	ax
	push	bx

	mov		bx,			[vga_param_y]
	and		bx,			0b0000_0001_1110_0000		; these bits correspond to the VRAM segment (0-15)
	shr		bx,			5
	in		ax,			VGA_REG						; read the VGA register
	and		ax,			0b11111111_11110000			; read the register, keep all bits except segment
	or		ax,			bx							; update segment bits
...
vga_get_pixel_mem_addr:
	; returns memory address as bx (within a 64K VRAM segment)
	; does not update VGA register for active segment (use vga_set_segment_from_xy)

	push	ax
	mov		bx,			[vga_param_y]
	and		bx,			0b00000000_00011111
	shl		bx,			11							; the lower 5 y bits correspond with the line number within VRAM segment
	mov		ax,			[vga_param_x]
	shl		ax,			1							; each x position = 2 bytes, so double bx
	or		bx,			ax							; yyyyyxxx_xxxxxxxx = address within VRAM 64K segment
	pop		ax
	ret
...
vga_draw_test_pattern:
	...
	.frame:
		...
		.red:
			mov		si,							0
			mov		word [vga_rect_start_x],	50
			mov		word [vga_rect_start_y],	50
			.redloop:
				mov		ax,		si
				shl		ax,		11
				mov		word [vga_param_color],		ax
				mov		ax,							[vga_rect_start_x]
				add		ax,							15
				mov		[vga_rect_end_x],			ax
				mov		ax,							[vga_rect_start_y]
				add		ax,							21
				mov		[vga_rect_end_y],			ax
				call	vga_draw_rect
				add		word [vga_rect_start_x],	16
				inc		si
				cmp		si,							32
				jne		.redloop
		...
		.full:
			...
			mov	word [vga_param_y],		180
			mov	word [vga_param_x],		64
			mov word [vga_param_color],	0b00000_000000_00000
			.full_loop_columns:
				mov			ax,						di
				shl			ax,						9
				or			ax,						si
...
vga_draw_pixel:
	; draws a pixel at x,y pixel position with color
	; uses vga_param_x, vga_param_y, vga_param_color
	...
	call	vga_set_segment_from_xy				; register updated in called routine
...
draw_pixel:
	...
	; TO DO update calling code to directly update vga_param_x/y and remove following temp assignments

	mov		ax,				[cursor_pos_h]
	add		ax,				[pixel_offset_h]
	mov		[vga_param_x],	ax

	mov		ax,				[cursor_pos_v]
	add		ax,				[pixel_offset_v]
	mov		[vga_param_y],	ax
	...
	mov		word es:[bx],	0xffff			; to do: pull color from var
