...
; Latest update: .rodata section
; Assembler: NASM
; Target processor clock: Tested with 6 MHz processor clock on breadboard build
;
; *physical memory map*
; -----------------------
; -    ROM  (256 KB)    -
...
send_garbage:
	; when ching CS in SPI, a byte of (any) data should be sent just prior to and just following the CS change -- calling this a garbage byte
...
test_math_co:
	...
	mov		ax,		[AREA+3]				; should be 0x46
	call	print_char_hex

	mov		ax,		[AREA+2]				; shoulde be 0xce
	call	print_char_hex

	mov		ax,		[AREA+1]				; shoulde be 0x40
	call	print_char_hex

	mov		ax,		[AREA]					; shoulde be 0x0c
	call	print_char_hex
