...
; Latest update: KBD buffer clean-up, Interrupt 10H start
; Assembler: NASM
...
section .data
	...
	; VARs
	varstart:
		;msg_BIOS_upcomplete		db		'BIOS update complete! Please reset the system.', 0x0
		...
		kb_flags				db		0x0					; track status of keyboard input
		kb_wptr					db		0x0					; keyboard buffer write pointer
		kb_rptr					db		0x0					; keyboard buffer read pointer
		kb_buffer times 256		db      0x0					; 256-byte keyboard buffer
		...
		mouse_pos_x_prev		dw		0x0
		mouse_pos_y_prev		dw		0x0
		...
		os_buffer	times 256	db      0x0					; 256-byte buffer for operating system keyboard input (i.e., commands)
...
setup_interrupts:
	...
	; ***** Mouse *****
		mov word [MOUSE_IVT_OFFSET], mouse_isr	; DS set to 0x0000 above. These MOVs are relative to DS.
												; 0x0000:0x0024 = IRQ1 offset in IVT
		mov		ax, 0xC000
		mov word [MOUSE_IVT_OFFSET+2], ax		; 0x0000:0x0026 = IRQ1 segment in IVT
...
isr_video:
	...
	; 0Ah = write character (no color specified)
		; al = char
		; bh = page number						-ignore page number and write to both pages
		; cx = num times to print				-ignore for now
		
		cmp		ah,		0x0a
		jne		.out				; move to next check, or if last check, get out
		call	print_char_vga				; 	[print_char_options] bit0=0 print both pages, bit0=1 print only one page
...
shadow_start:
	...
	; get to char data (assuming it wasn't moved in BIOS update, otherwise might see garbage on VGA output)
	mov		ax, 0xf000			; Read-only data in ROM at 0x30000 (0xf0000 in address space  0xc0000+0x30000). 
									; Move es to this by default to easy access to constants.
	mov		es,	ax				; extra segment
...
vga_init_color:
	; ** Currently an unused procedure -- including for future use/reference
	; ** Applies to 320x240 VGA dual-port SRAM video card
	; ** Thanks to @Damouze!
	;
	; vga_init_color: initialize VRAM with a specific color
	; 
	; In:			AL = color to write to the screen
	; Return:		Nothing
	; 
	...
	push	di
	push	es
	pushf						; Save flags (we're setting the direction flag later)

	mov		ah, al				; Copy contents of AL to AH
								; (i.e., 0xffff down to 0x0000)
	mov		di, 0xb000			; segment start 
								; (i.e., 0xb000 as top)
	std							; Set the direction flag, DI will automatically decrented after each write
	mov		cx, 2				; The loop will run twice

	.again:
		push	cx
		mov		es, di
		mov		di, 0xfffe			; offset within segment
		mov		cx, 0x8000			; This is the number of write operations
	
		rep		stosw				; Repeatedly store CX times
		mov		di, 0xa000			; Set DI to segment number of bottom half of VRAM
		pop		cx
		loop	.again
...
spi_writebyte_port_a_slow:
	...
	.clock:
		...
		push	bp
		push	si
		mov		bp,	0x0005				; low word of counter
...
spi_readbyte_port_a_slow:
	...
	.loop:
		...
		push	bp
		push	si
		mov		bp,	0x0009				; low word of counter
		mov		si,	0x0001				; high word of counter
		call	delay_configurable		; counts down from dword to zero
		pop		si
		pop		bp	
...
spi_getMouseData:
	...
	mov		al,				0b11001100		; get mouse data CMD #
...
draw_mouse_pointer:
	...
	; print coord to 2nd line of lcd -- first line has raw mouse data
	;call	lcd_line2
	;mov		ax, [mouse_pos_x]
	;xchg	ah, al
	;call	print_char_hex
	...
	; erase old cursor on alternate video frame
	mov		dx, [mouse_pos_x_prev]
	mov		di, [mouse_pos_y_prev]
	mov		bx, 4
	mov		ax,	0x0000
	call	vga_draw_circle_filled
	; do not swap frames... next draw of mouse cursor will draw new and swap
	; save latest location as prev location so that it can be erased next time
	mov		ax, [mouse_pos_x]
	mov		[mouse_pos_x_prev], ax
	mov		ax, [mouse_pos_y]
	mov		[mouse_pos_y_prev], ax
...
cmd_help:
	push	word [vga_param_color]
	mov		word [vga_param_color],		0b10000_000000_11111		; blue
	push	bx
...
times 0x30000-($-$$)-0x0a00 db 0xff	; Fill much of ROM with FFs to allow for faster writing of flash ROMs
...
section .rodata start=0x30000
	...
	; strings			; terminate with 0x0
		...
		msg_vga_post_version						db	'rehsd -- BIOS version 0.0000.0002 -- 2 March 2023', 0x0
		...
		msg_cmd_help1								db  'Supported commands: help, ver, cls, updatebios', 0x0
		msg_cmd_help2								db  '                                              ', 0x0
		msg_update_bios								db	'Updating BIOS...',0x0
