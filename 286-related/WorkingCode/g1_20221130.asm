...
; -locate SPI timing issue on cold boot for 8char7seg display
...
section .data
	...
	ivt		times 1024	db	0				; there has to be a better way to do this
											; trying to prevent writing in the same space as the IVT
...
spi_sdcard_init:
	...
	call	print_string_to_serial
													; Each cmd has six bytes of data to be sent
	mov		word [cmd0_bytes],			0x4000		; GO_IDLE_STATE
	mov		word [cmd0_bytes+2],		0x0000
	mov		word [cmd0_bytes+4],		0x0095

	mov		word [cmd1_bytes],			0x4100		; SEND_OP_COND
	mov		word [cmd1_bytes+2],		0x0000		
	mov		word [cmd1_bytes+4],		0x00f9

	mov		word [cmd8_bytes],			0x4800		; SEND_IF_COND
	mov		word [cmd8_bytes+2],		0x0001		
	mov		word [cmd8_bytes+4],		0xaa87

	mov		word [cmd12_bytes],			0x4C00		; STOP_TRANSMISSION
	mov		word [cmd12_bytes+2],		0x0000		
	mov		word [cmd12_bytes+4],		0x0061

	mov		word [cmd18_bytes],			0x5200		; READ_MULTIPLE_BLOCK, starting at 0x0
	mov		word [cmd18_bytes+2],		0x0000		
	mov		word [cmd18_bytes+4],		0x00e1

	mov		word [cmd41_bytes],			0x6940		; SD_SEND_OP_COND
	mov		word [cmd41_bytes+2],		0x0000		
	mov		word [cmd41_bytes+4],		0x0077	

	mov		word [cmd55_bytes],			0x7700		; APP_CMD
	mov		word [cmd55_bytes+2],		0x0000		
	mov		word [cmd55_bytes+4],		0x0065
...
spi_send_NanoSerialCmd:
	...
	pop		ax						; get back original ax
	push	ax						; save it again to stack
	call	spi_writebyte_port_a			; using original al
